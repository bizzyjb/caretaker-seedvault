...
- **Linux: the game moved its saves and the monitor did not follow.** The Last Caretaker
  update of 31 August 2026 writes to `SaveGames/LocalSteamUser/VoyageSaveGame_*.sav`
  instead of `SaveGames/<SteamID>/<SteamID>_*.sav`. The old folder is left behind intact,
  so an installed monitor kept watching it and quietly captured nothing. It now watches
  every profile folder beside the one it was given and rechecks while running, restores
  under the name the game currently uses, and reports a move in `status`. Two further
